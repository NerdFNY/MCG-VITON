...
def ready_arguments(fname_or_dict):
    ...
    if want_shapemodel:
        ...
        v_posed = dd['v_posed']
        J_tmpx = MatVecMult(dd['J_regressor'],v_posed[:,0])
        J_tmpy = MatVecMult(dd['J_regressor'],v_posed[:,1])
        J_tmpz = MatVecMult(dd['J_regressor'],v_posed[:,2])
        dd['J'] = ch.vstack((J_tmpx, J_tmpy, J_tmpz)).T
    else:    
...
def load_model(fname_or_dict):
    ...
    # kintree_table运动学树 bs_style 蒙皮方式
