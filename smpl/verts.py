...
#此文件定义SMPL加载程序的基本蒙皮模块，SMPL加载程序定义骨骼和混合形状对模板网格顶点的影响。

Modules included:
- verts_decorated: 
  creates an instance of the SMPL model which inherits model attributes from another SMPL model.
  #创建SMPL模型的实例，该实例从另一个SMPL模型继承模型属性。
...
def verts_core(pose, v, J, weights, kintree_table, bs_style, want_Jtr=False, xp=chumpy):
    ...
    assert(bs_style=='lbs')
    result = smpl.lbs.verts_core(pose, v, J, weights, kintree_table, want_Jtr, xp)
